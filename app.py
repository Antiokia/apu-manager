...
/* ── HERO ── */
.hero {{
    background: linear-gradient(135deg, {BLUE} 0%, {BLUE2} 50%, #001f3d 100%);
    border-radius: 16px;
...
.hero-title {{
    ...
    color: #ffffff; line-height: 1; margin: 0 0 8px;
}}
.hero-sub {{
...
.kpi-label {{
    ...
    letter-spacing: 2px; color: {TEXT_SEC};
    text-transform: uppercase; margin-bottom: 8px;
}}
.kpi-value {{
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 32px; font-weight: 700;
    color: {KPI_VAL}; line-height: 1;
}}
.kpi-sub {{ font-size: 12px; color: {KPI_SUB}; margin-top: 4px; }}
...
.cat-dot {{ width: 10px; height: 10px; border-radius: 50%; background: white; flex-shrink: 0; }}
.cat-name {{
    font-family: 'Barlow Condensed', sans-serif;
...
.total-value {{
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 42px; font-weight: 800; color: {ACCENT};
}}
...
.stDownloadButton button:hover {{
    background: #ffc933 !important; transform: translateY(-1px);
}}
...
[data-testid="stFileUploaderDropzone"] label {{ color: {UPLOADER_LBL} !important; }}
[data-testid="stFileUploaderDropzoneInstructions"] {{ color: {TEXT_DIM} !important; }}

/* ── TABS ── */
.stTabs [data-baseweb="tab-list"] {{
    background: {TAB_BG};
    border-radius: 8px; padding: 4px; gap: 4px; margin-bottom: 20px;
    transition: background .3s;
}}
.stTabs [data-baseweb="tab"] {{
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 14px; font-weight: 600;
    letter-spacing: 1px; color: {TAB_CLR};
...
.items-row {{
    ...
    display: flex; gap: 24px; margin-bottom: 8px; align-items: center;
    transition: background .3s;
}}
.items-num {{
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 20px; font-weight: 700; color: {ACCENT}; width: 40px;
}}
.items-info {{ flex: 1; }}
.items-cant {{ font-size: 13px; color: {ITEMS_CANT}; }}
.items-base {{
    font-family: 'Barlow Condensed', sans-serif;
    font-size: 16px; font-weight: 600; color: {ITEMS_BASE};
}}
...
::-webkit-scrollbar-track {{ background: {SCROLL_TRK}; }}
...
# ── HERO HEADER ──────────────────────────────
col_hero, col_theme = st.columns([5, 1])
...
with col_theme:
    ...
    if st.button(label, key="toggle_theme", use_container_width=True):
...
with col_info:
    ...
    req_text   = TEXT_SEC
    st.markdown(f"""
    <div style="background:{req_bg}; border:1px solid {req_border}; border-radius:12px; padding:20px; margin-top:8px; transition:background .3s;">
        <div style="font-family:'Barlow Condensed',sans-serif; font-size:15px; font-weight:700; color:{ACCENT}; margin-bottom:12px; letter-spacing:1px;">REQUISITOS DEL ARCHIVO</div>
        <div style="font-size:13px; color:{req_text}; line-height:1.8;">
            ✓ &nbsp;Hoja <b style="color:{TEXT_PRI}">PPTO</b> con ítems y cantidades<br>
            ✓ &nbsp;Hojas <b style="color:{TEXT_PRI}">item 1</b>, <b style="color:{TEXT_PRI}">item 2</b>… con APU<br>
            ✓ &nbsp;Hoja <b style="color:{TEXT_PRI}">AUX</b> con tabla de categorías<br>
            ✓ &nbsp;Formatos <b style="color:{TEXT_PRI}">.xlsm · .xlsx · .xls</b>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if drive_configurado() and not _tiene_refresh_token():
    st.markdown(f"""
    <div style="background:{SURFACE}; border:1px solid {ACCENT}; border-radius:10px;
                padding:20px 24px; margin-bottom:24px;">
        <div style="font-family:'Barlow Condensed',sans-serif; font-size:16px;
                    font-weight:700; color:{ACCENT}; margin-bottom:8px; letter-spacing:1px;">
            🔑 AUTORIZACIÓN DE GOOGLE DRIVE PENDIENTE
        </div>
        <div style="font-size:13px; color:{TEXT_SEC}; margin-bottom:16px;">
            Solo debes hacer esto <b style="color:{TEXT_PRI}">una vez</b>.
    ...
    with col_auth1:
        ...
        if "auth_url" in st.session_state:
            st.markdown(f"""
            <div style="background:{BG}; border:1px solid {BORDER2}; border-radius:8px;
                        padding:12px; margin-top:8px; word-break:break-all;">
                <div style="font-size:11px; color:{TEXT_SEC}; margin-bottom:6px;">Abre este link en tu navegador:</div>
                <a href="{st.session_state['auth_url']}" target="_blank" style="color:#58a6ff; font-size:12px;">
                    🔗 Abrir autorización de Google
                </a>
            </div>
            """, unsafe_allow_html=True)
    with col_auth2:
        codigo = st.text_input("2️⃣  Pega aquí el código que te dio Google", placeholder="4/0AX4XfWi...")
...
if archivo is not None:
    try:
        ...
        with tab1:
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <div style="font-family:'Barlow Condensed',sans-serif; font-size:22px; font-weight:700; color:{TEXT_PRI};">
        ...
        with tab2:
            ...
            st.markdown(f"<div style='margin-top:24px; margin-bottom:8px; font-family:Barlow Condensed,sans-serif; font-size:16px; font-weight:700; color:{TEXT_SEC}; letter-spacing:2px;'>DISTRIBUCIÓN</div>", unsafe_allow_html=True)
            ...
            for cat_nombre, subtotal in subtotales.items():
                ...
                <div style="margin-bottom:10px;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                        <span style="font-size:13px; color:{TEXT_PRI};">{cat_nombre}</span>
                        <span style="font-size:13px; font-weight:600; color:{ACCENT};">{pct:.1f}%</span>
                    </div>
                    <div style="background:{BORDER}; border-radius:4px; height:8px; overflow:hidden;">
                        <div style="width:{pct:.1f}%; height:100%; background:rgb({r},{g},{b}); border-radius:4px;"></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with tab3:
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='font-family:Barlow Condensed,sans-serif; font-size:16px; color:{TEXT_SEC}; letter-spacing:2px; margin-bottom:16px;'>ÍTEMS PROCESADOS DEL PRESUPUESTO</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="text-align:center; padding:80px 0; color:{TEXT_DIM};">
        ...
        <div style="font-size:14px; margin-top:8px; color:{TEXT_DIM};">
